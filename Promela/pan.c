...
{	int j = 0, h = now._nr_pr;
	...
	case 0:	/* LinearRegression */
		...
		/* locals: */
#ifdef VAR_RANGES
		logval("LinearRegression:m", ((P0 *)pptr(h))->m);
		logval("LinearRegression:b", ((P0 *)pptr(h))->b);
#endif
#ifdef HAS_CODE
		locinit0(h);
#endif
...
iniglobals(int calling_pid)
{
	...
	{	int l_in;
		...
		{
			logval("X[l_in]", now.X[l_in]);
		}
	}
	{	int l_in;
		for (l_in = 0; l_in < 10; l_in++)
		{
			logval("Y[l_in]", now.Y[l_in]);
		}
	}
...
{	/* int i; */
	switch(tp) {
	case 0:
		printf("local vars proc %d (LinearRegression):\n", pid);
	printf("	int    m:	%d\n", ((P0 *)pptr(pid))->m);
	printf("	int    b:	%d\n", ((P0 *)pptr(pid))->b);
		break;
	}
